...
export default function ProjectsPage() {
  ...
  async function load() {
    try {
      ...
      setDepartments(departmentResponse.ok ? await departmentResponse.json() : []);
    } catch (loadError) {
  ...
  return <div className="max-w-6xl">
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h1 className="text-2xl font-bold tracking-tight">Projects</h1><p className="mt-1 text-sm text-muted-foreground">Multi-day outcomes with linked, automatically allocated work items.</p></div>
    ...
    {projects.length === 0 ? <EmptyState title="No projects yet" description="Create a project for work that needs a shared outcome, timeframe, and multiple work items." /> : <div className="grid gap-4 lg:grid-cols-2">
      {projects.map((project) => {
        ...
        return <article key={project.id} className="rounded-lg border border-border bg-card p-5">
